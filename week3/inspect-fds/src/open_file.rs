...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let file_name = Self::path_to_name(fs::read_link(format!("/proc/{}/fd/{}", pid, fd)).ok()?.to_str()?);
        let file_info_str = fs::read_to_string(format!("/proc/{}/fdinfo/{}", pid, fd)).ok()?;
        Some(OpenFile::new(file_name, Self::parse_cursor(&file_info_str)?, Self::parse_access_mode(&file_info_str)?))
    }
