...
fn main() {
    ...
    let tgproc = match ps_utils::get_target(target).expect("Error calling ps") {
        Some(proc) => proc,
        None => {
            eprintln!("Target \"{}\" did not match any running PIDs", target);
            std::process::exit(1);
        }
    };
    println!("Found pid {}", tgproc.pid);
    tgproc.print();
    let child_processes = ps_utils::get_child_processes(tgproc.pid).expect("Error getting child pids");
